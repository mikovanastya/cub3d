...
/*   Updated: 2022/07/27 14:18:45 by rtwitch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_col_img(t_column *col, t_vect *ray, t_img *img)
{
	...
	{
		img->data[i * RES_X + col->x_pos] = get_fog_color(FOG_ON, \
			get_pixel(col->texture, tex_x, tex_y), ray->dist);// !! ставим по координатам пиксель с tex (с учетом тумана)
		i++;
		tex_y += step_tex;
	}
}
...
int	get_real_wall_type(t_vect **rays, int x_pos)
{
	...
	cur_type = get_wall_type(rays[x_pos]);
	if ((x_pos > 2) && (x_pos < RES_X - 1))
	{
		left_type = get_wall_type(rays[x_pos - 1]);
		right_type = get_wall_type(rays[x_pos + 1]);
...
void	render_column(t_env *env, t_img *img, t_vect **rays, int x_pos)
{
	...
	column.wall_height = RES_Y / (2 * cosf(env->player.angle \
		- rays[x_pos]->angle) * tan(VFOV) * rays[x_pos]->dist);// получение высоты колонки

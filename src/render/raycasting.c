...
/*   Updated: 2022/07/27 14:12:44 by rtwitch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "render.h"

void	init_raycast_steps(t_raycast *rc, t_vect dir)
{
	if (dir.x < 0)// distance from the ray starting position to the first side to the above / below,
	{
		rc->step.x = -1.0f;
		rc->side_dist.x = (rc->player_vect.x - rc->map_check.x) \
			* rc->delta_dist.x;
	}
	else
	{
		rc->step.x = 1.0f;
		rc->side_dist.x = (rc->map_check.x + 1 - rc->player_vect.x) \
			* rc->delta_dist.x;
	}
	if (dir.y < 0)
	{
		rc->step.y = -1.0f;
		rc->side_dist.y = (rc->player_vect.y - rc->map_check.y) \
			* rc->delta_dist.y;
	}
	else
	{
		rc->step.y = 1.0f;
		rc->side_dist.y = (rc->map_check.y + 1 - rc->player_vect.y) \
			* rc->delta_dist.y;
	}
}
...
{
	rc->player_vect = vect_new(env->player.x / env->size, \
			env->player.y / env->size);// позиция игрока на поле
...
void	find_wall(t_env *env, t_raycast *rc)
{
	while (!rc->is_wall_found)
	{
		...
		if (ft_strchr(" 1", env->map->grid[(int)rc->map_check.y] \
			[(int)rc->map_check.x]))
...
t_vect	**raycasting(t_env *env)
{
	...
	while (i < RES_X)// проходим все по ширине
	{
		...
		*ray = get_ray(env, vect_rot(vect_new(-1.0f, 0.0f), \
			env->player.angle + cur_angle));// получаем луч попадания в стену
